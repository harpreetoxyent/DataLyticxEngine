...
public class ReteWorkingMemory implements IWorkingMemory {
	...
	public void assertObject(Object ob) throws RuleComponentException {
		...
		if (null != node) {
			...
			RuleComponent.logger.log(0, ob
					+ " type object type node doen't exist");
		}
	}
	 ...
	 */
	private ObjectTypeNode getObjectTypeNode(Object ob)
			throws RuleComponentException {
		HashMap<String, ObjectTypeNode> objectTypeNodeList = rete.getReteNode()
				.getObjectTypeList();
			...
			if (node instanceof TerminalNode) {
				...
				while (node instanceof JoinNode) {
					...
					if (jn.getBinder().equals(ConditionConstants.ANDCONDITION)) {
						isRuleTrue = validate(jn.getRightInput(), node)
								&& validate(jn.getLeftInput(), node);
						if (isRuleTrue)
	...
	private boolean validate(INode inputNode, INode node) {
		...
		if (inputNode instanceof ConditionIdentifier) {
			...
			if (this.participantConditionsList.contains(condIdef.getCondition()
					.getConditionString())) {
				isRuleTrue = true;
			} else {
				isRuleTrue = false;
	...
	private boolean validateJoinNodeCondition(INode node) {
		...
		if (node instanceof JoinNode) {
			jn = (JoinNode) node;
			if (jn.getBinder().equals(ConditionConstants.ANDCONDITION))
				isRuleTrue = validateJoinNodeCondition(jn.getLeftInput())
						&& validateJoinNodeCondition(jn.getRightInput());
			else
				isRuleTrue = validateJoinNodeCondition(jn.getLeftInput())
						|| validateJoinNodeCondition(jn.getRightInput());
		} else {
			if (node instanceof ConditionIdentifier) {
				...
				if (this.participantConditionsList.contains(condIdef
						.getCondition().getConditionString())) {
					isRuleTrue = true;
				} else {
					isRuleTrue = false;
				}
	 ...
	 * @param ob
	 */
	private void checkCondition(AlphaNode alphaNode, Object ob)
			throws RuleComponentException {
		...
		try {
			// For methods
			Iterator<ConditionConstraints> constraintsIter = alphaNode
					.getConditionConstraintTable().values().iterator();
			while (constraintsIter.hasNext()) {
				...
				if (nameOfCondition.indexOf(RulesConstants.GET) == 0) {
					...
					if (nameOfCondition.indexOf('(') > 0
							&& nameOfCondition.indexOf(')') > 0) {
				...
				result = checkIfFactAlreadyPresent(nameOfCondition, value,
						alphaNode);
			...
			for (int i = 0; i < fields.length; i++) {
				...
				if (dataType.toString().equals(
						EvaluatorConstants.PRIMITIVE_BOOLEAN_TYPE)) {
					methodName = RulesConstants.IS
							+ changeFirstUpper(fields[i].getName());
					try {
						value = (Object) clazz.getDeclaredMethod(methodName,
								null).invoke(ob, null);
					} catch (Exception e) {
						methodName = RulesConstants.GET
								+ changeFirstUpper(fields[i].getName());
						value = (Object) clazz.getDeclaredMethod(methodName,
								null).invoke(ob, null);
					}
			...
			if (conditionString.charAt(i) == '='
				...
				if (conditionString.charAt(nextDelim) == '=') {
					...
					delimitter = "" + conditionString.charAt(i)
							+ conditionString.charAt(nextDelim);
					break;
				} else {
					idenf[0] = conditionString.substring(0, i);
	...
	private List validateBetaCondition() {
		// RuleComponent.logger.log(0,"participantConditionsList size = "
		// +participantConditionsList.size());
	 ...
	 */
	private void validateAlphaCondition(String noc, Object value,
			AlphaNode alphaNode) throws RuleComponentException {
		String operator, type, condValue;
		Class dataType;
		boolean result;
		Hashtable conditionConstraintList = alphaNode
				.getConditionConstraintTable();
		try {
			for (int i = 0, j = 1; i < conditionConstraintList.size(); i++, j++) {
				ConditionConstraints condConstraint = (ConditionConstraints) conditionConstraintList
						.get(new Integer(j));
				if ((condConstraint.getNameOfCondition().trim())
						.equalsIgnoreCase(noc)) {
					...
					// RuleComponent.logger.log(0,"inside validate alpha condition for noc = "
					// +noc + " value = " +condValue);

					// RuleComponent.logger.log(0,"[ReteWorkingMemory][checkCondition][value]".concat(value
					// == null ? "NULL" : value.toString()));
					// RuleComponent.logger.log(0,"[ReteWorkingMemory][checkCondition][condValue]".concat(condValue
					// == null ? "NULL" : condValue.toString()));
					// RuleComponent.logger.log(0,"[ReteWorkingMemory][checkCondition][dataType]".concat(dataType
					// == null ? "NULL" : dataType.getName()));
					// RuleComponent.logger.log(0,"[ReteWorkingMemory][checkCondition][operator]".concat(operator
					// == null ? "NULL" : operator.toString()));
					result = isConditionSatisfied(dataType, value, condValue,
							operator);

					if (result) {
						participantConditionsList.add(condConstraint
								.getConditionString());
						// RuleComponent.logger.log(0,"name of condition = " +
						// condConstraint.getNameOfCondition() + " value = " +
						// condValue);
						alphaNode.addToFactsList(new Fact(condConstraint
								.getNameOfCondition().trim(), condValue,
								condConstraint.getConditionString()));
					}
				}
			}
		} catch (Exception e) {
	 ...
	 * @return
	 */
	private boolean checkIfFactAlreadyPresent(String noc, Object value,
			AlphaNode alphaNode) {
	...
	/**
	 * Executes all the rules matched. the execution of rules happen based on
	 * salience value
	 * @throws RuleComponentException 
	 * 
	 */
	public List<IRuleClass> executeRules(Object[] inObject) throws IllegalAccessException, RuleComponentException{
		Class inObjectClass = inObject[0].getClass();
		List<IRuleClass> ruleList = new ArrayList<IRuleClass>();
		for (Iterator it = this.action.getTerminalNodeList().iterator(); it
				.hasNext();) {
		...
		for (Iterator it = sortedKeys.iterator(); it.hasNext();) {
			...
			for (Iterator it1 = ruleClassList.iterator(); it1.hasNext();) {
				...
				List variableList = ruleClass.getVariablesList();
				if (variableList != null) {
					for (Iterator variableIterator = variableList.iterator(); variableIterator
							.hasNext();) {
						Variables keyVariable = (Variables) variableIterator
								.next();
						String typeVariableName = keyVariable.getType();

						String factTypeName = inObject[0].getClass().getName();
						if (typeVariableName.equals(factTypeName)) {
							// Set the incoming fact data into the rule object
							// returned to 3rd party systems
							Field[] fields = ruleClass.getClass().getDeclaredFields();
							
							for (Field field : fields) {								
								if(field.getType().getName().equals(factTypeName))
								{
									field.set(ruleClass, inObject[0]);
								}
							}
						}
						// Set the inward fact on which condition is met to rule
						// alpha object
					}
				}
				ruleClass.consequence();
				conseqList = ruleClass.getConsequenceList();
				for (Iterator conseqItr = conseqList.iterator(); conseqItr
						.hasNext();) {
	...
	/**
	 * modifies the fact present in rete tree Modify happens in 2 step : retract
	 * and assert
	 * 
	 * @param noc
	 * @param value
	...
	public IFactHandle modifyObject(Object ob) throws RuleComponentException {
		...
		if (null != node) {
			...
			for (Iterator it = l1.iterator(); it.hasNext();) {
				Fact fact = (Fact) it.next();
				// RuleComponent.logger.log(0,"fact, noc = "
				// +fact.getNameOfCondition() + " value = " +fact.getValue());
			}
	 ...
	 * @param ob
	 */
	private void retractFactsFromList(AlphaNode alphaNode, Object ob)
			throws RuleComponentException {
		Class clazz = ob.getClass();
		Field[] fields = clazz.getFields();
		String methodName, nameOfCondition;
	 ...
	 * @param ob
	 */
	private void assertFactInFactList(AlphaNode alphaNode, Object ob)
			throws RuleComponentException {
		Class clazz = ob.getClass();
		Field[] fields = clazz.getFields();
		String methodName, nameOfCondition;
		...
		try {
			for (int i = 0; i < fields.length; i++) {
				...
				alphaNode
						.addToFactsList(new Fact(nameOfCondition, value, null));
	 ...
	 * @return
	 */
	private boolean isConditionSatisfied(Class dataType, Object value,
			String condValue, String operator) throws Exception {
		boolean result = false;
		...
		if (dataType.getSimpleName().equals(
				EvaluatorConstants.PRIMITIVE_FLOAT_TYPE)) {
			evaluator = PrimitiveFloatEvaluator
					.getPrimitiveFloatEvaluator(operator);
			if (evaluator != null)
				result = evaluator.evaluate(value, condValue);
		} else if (dataType.getSimpleName().equals(
				EvaluatorConstants.PRIMITIVE_INT_TYPE)) {
			evaluator = PrimitiveIntEvaluator
					.getPrimitiveIntEvaluator(operator);
			if (evaluator != null)
				result = evaluator.evaluate(value, condValue);
		} else if (dataType.getSimpleName().equals(
				EvaluatorConstants.PRIMITIVE_LONG_TYPE)) {
			evaluator = PrimitiveLongEvaluator
					.getPrimitiveLongEvaluator(operator);
			if (evaluator != null)
				result = evaluator.evaluate(value, condValue);
		} else if (dataType.getSimpleName().equals(
				EvaluatorConstants.PRIMITIVE_SHORT_TYPE)) {
			evaluator = PrimitiveShortEvaluator
					.getPrimitiveShortEvaluator(operator);
			if (evaluator != null)
				result = evaluator.evaluate(value, condValue);
		} else if (dataType.getSimpleName().equals(
				EvaluatorConstants.PRIMITIVE_BYTE_TYPE)) {
			evaluator = PrimitiveByteEvaluator
					.getPrimitiveByteEvaluator(operator);
			if (evaluator != null)
				result = evaluator.evaluate(value, condValue);
		} else if (dataType.getSimpleName().equals(
				EvaluatorConstants.PRIMITIVE_DOUBLE_TYPE)) {
			evaluator = PrimitiveDoubleEvaluator
					.getPrimitiveDoubleEvaluator(operator);
			if (evaluator != null)
				result = evaluator.evaluate(value, condValue);
		} else if (dataType.getSimpleName().equals(
				EvaluatorConstants.PRIMITIVE_BOOLEAN_TYPE)) {
			evaluator = PrimitiveBooleanEvaluator
					.getPrimitiveBooleanEvaluator(operator);
			if (evaluator != null)
				result = evaluator.evaluate(value, condValue);
		} else if (dataType.getSimpleName().equals(
				EvaluatorConstants.STRING_TYPE)) {
			...
			if (null != evaluator)
				...
				EvaluatorConstants.DOUBLE_TYPE)) {
		...
		/*
		 * else if
		 * (dataType.getSimpleName().equals(EvaluatorConstants.CHAR_TYPE)) {
		 * evaluator = CharacterEvaluator.getCharacterEvaluator(operator); if
		 * (null != evaluator) result = evaluator.evaluate(value, new
		 * Character(condValue)); }
		 */
		else if (dataType.getSimpleName()
				.equals(EvaluatorConstants.OBJECT_TYPE)) {
			evaluator = new ObjectEvaluator(operator);
			if (null != evaluator)
				result = evaluator.evaluate(value, condValue);
		} else if (dataType.getSimpleName().equals(
				EvaluatorConstants.BOOLEAN_TYPE)) {
