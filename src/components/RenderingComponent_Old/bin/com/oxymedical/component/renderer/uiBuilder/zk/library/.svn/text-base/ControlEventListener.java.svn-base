package com.oxymedical.component.renderer.uiBuilder.zk.library;

import java.util.Hashtable;
import java.util.List;

import org.zkoss.zk.ui.event.Event;
import org.zkoss.zk.ui.event.EventListener;
import org.zkoss.zk.ui.event.Events;
import org.zkoss.zul.Textbox;
import org.zkoss.zul.Window;
import org.zkoss.zul.Combobox;

import com.oxymedical.component.renderer.RendererComponent;
import com.oxymedical.component.renderer.command.UiLibraryCompositeCommand;
import com.oxymedical.core.commonData.IData;
import com.oxymedical.core.commonData.IHICData;
import com.oxymedical.core.router.IDataUnitRouter;
import com.oxymedical.core.router.IRouter;

public class ControlEventListener {

	String method ="executeList";
	String componentId = "dbComponent";
	String classname = "com.oxymedical.component.db.DBComponent";
	IDataUnitRouter router = (IDataUnitRouter)RendererComponent.dataUnitRouter;
	private Hashtable formValues = new Hashtable();
	UiLibraryUtil uiLibraryUtil=new UiLibraryUtil();
	
	public void getComboData(String dataPatternId, String formPatternId, Window rootFormValue, String paramList, Combobox comboObj)
	{
	    UiLibraryCompositeCommand command=new UiLibraryCompositeCommand();
		command.setMethodName(method);
		command.setRouter(router);
	    command.setClassname(classname);
		command.setComponentId(componentId);
	    command.setDataPatternId(dataPatternId);
	    command.setFormPatternId(formPatternId);
	    command.setFormValues(formValues);
		command.setRootFormValue(rootFormValue);
		command.setParamList(paramList);
		command.setSession(null);
		command.setComboSelectedValue(null);
		command.setValidListRequest(false);
		command.setPagingId("");
		command.execute();
		IHICData hicData = command.getHICData();
		List listValue = null;
		IData dataUnit = hicData.getData();
		listValue = dataUnit.getQueryData().getListData();
		String[][] allValues = dataUnit.getQueryData().iterateListData(listValue);
		uiLibraryUtil.showComboData(allValues,comboObj);
	}

	public void onEvent(Combobox target) 
	{
			if (target == GridRowCustomRenderer.methodCombobox)
				getComboData("Fortis" , "nursing_progress_notes" , GridRowCustomRenderer.window ,"get MasterMethod.methodId,MasterMethod.methodName from FORTIS.MASTERMETHOD",GridRowCustomRenderer.methodCombobox);
	
			if(target == GridRowCustomRenderer.feedTypeCombobox)
				getComboData("Fortis" , "nursing_progress_notes" , GridRowCustomRenderer.window ,"get MasterFeedType.FeedTypeId,MasterFeedType.FeedName from FORTIS.MasterFeedType",GridRowCustomRenderer.feedTypeCombobox);
			
			if(target == GridRowCustomRenderer.ivFluidsCombobox)
				getComboData("Fortis" , "nursing_progress_notes" , GridRowCustomRenderer.window ,"get MasterIVFluid.IVFluidId,MasterIVFluid.IVFluidName from FORTIS.MasterIVFluid",GridRowCustomRenderer.ivFluidsCombobox);
			
			if(target == GridRowCustomRenderer.boCombobox)
				getComboData("Fortis" , "nursing_progress_notes" , GridRowCustomRenderer.window ,"get MasterBO.BOId,MasterBO.BOName from FORTIS.MasterBO",GridRowCustomRenderer.boCombobox);
			
			if(target == GridRowCustomRenderer.FIO2Combobox)
				getComboData("Fortis" , "nursing_progress_notes" , GridRowCustomRenderer.window ,"get MasterFIO2.FIO2Id,MasterFIO2.FIO2name from FORTIS.MasterFIO2",GridRowCustomRenderer.FIO2Combobox);
			
			if(target == GridRowCustomRenderer.tempIncBedCombobox)
				getComboData("Fortis" , "nursing_progress_notes" , GridRowCustomRenderer.window ,"get MasterTempIncBed.TempIncBedId,MasterTempIncBed.TempIncName from FORTIS.MasterTempIncBed",GridRowCustomRenderer.tempIncBedCombobox);
	}
	public void onEvent(Textbox target)
	{
		System.out.println("*****Inside Control Listener for IVTotal textbox*****");
		
		if (target == GridRowCustomRenderer.ivTotalTextbox)
		{
			Double total = 0.0;
			Double ivPerHourData = (new Double(GridRowCustomRenderer.ivPerHourTextbox.getValue()));
			Double aaPerHourData = (new Double(GridRowCustomRenderer.aaPerHourTextbox.getValue()));
			Double lipidPerHourData = (new Double(GridRowCustomRenderer.lipidPerHourTextbox.getValue()));
			total = total + ivPerHourData + aaPerHourData + lipidPerHourData;
			GridRowCustomRenderer.ivTotalTextbox.setValue(String.valueOf(total));
		}
		
		
	}
}